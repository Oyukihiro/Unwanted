...
class YtOauthView(disnake.ui.View):
            ...
            async with self.bot.session.post('https://oauth2.googleapis.com/token', data=payload) as response:
                ...
                if response.status != 200:
                    ...
                    if response_data["error"] != "authorization_pending":
                        self.exception_txt = f"**Failed to wait for account authorization:** `({response.status}) - {response_data['error_description']}`"
                        return
    ...
    async def send_authurl_callback(self, interaction: disnake.MessageInteraction):
        ...
        await interaction.send(embed=disnake.Embed(
            ...
            description=f"**Link to authenticate Google account:**"
                        f" ```\n{verification_url}``` "
                        "`If you have already authorized the application you must wait up to 15 seconds for "
                        "this message to update confirming the process.`"),
...
class YtOauthLL(commands.Cog):
    ...
    async def oauth_command(self, ctx: CustomContext):
        ...
        embed = disnake.Embed(
            color=color,
            description=f"## Get refresh-token from a Google account\n\n"
                        f"⚠️ **Caution!** Use (or create) a disposable account instead of proceeding with a personal use "
                        f"account due to the high chance of the account being banned by Google (avoid using accounts that may contain a phone number or recovery email, if you create one now avoid putting a phone number and recovery email)."
        )
        ...
        if not (refresh_token:=view.data.get('refresh_token')):
            raise GenericError("**Time to link Google account expired!**")
        ...
        embed = disnake.Embed(
            ...
            description=f"## Account Confirmation:\n"
                        ...
                        f"**Name:** ```ansi\n [31;1m{name} [0m``` "
                        "⚠️ Caution! If this is a personal use account, click the \"No\" button and use (or create) a disposable account!"
        ...
        if self.bot.pool.mongo_database:
            try:
                ...
                txts.append(f"Failed to save refreshToken to MongoDB: {repr(e)}")
        ...
        if os.path.isfile("./application.yml"):
            ...
            try:
                ...
                if (node := self.bot.music.nodes.get("LOCAL")) and "youtube-plugin" in node.info["plugins"]:
                    ...
                    if resp.status != 204:
                        txts.append(f"Error applying refreshToken to LOCAL lavalink: {resp.status} - {await resp.text()}")
                    else:
                        txts.append("The refreshToken has been automatically configured on the LOCAL lavalink server")

                else:
                    txts.append("The refreshToken has been successfully added to application.yml!")

            except Exception as e:
                traceback.print_exc()
                txts.append(f"Error saving refreshToken to application.yml: {repr(e)}")

        txts.append("Avoid showing this token publicly!")

        await func(embed=disnake.Embed(
            color=color,
            description=f"### Google account refreshToken successfully obtained!\n```{refresh_token}``` "
