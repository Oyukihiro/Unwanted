...
class LastFMView(disnake.ui.View):
    ...
    def build_components(self):
        ...
        if self.session_key:
            ...
            btn1 = disnake.ui.Button(label=f"{'Disable' if self.scrobble_enabled else 'Enable'} scrobbling/logging")
    ...
    async def scrobble_enable(self, interaction: disnake.MessageInteraction):
        ...
        if (retry_after := self.cooldown_scrobble_btn.get_bucket(interaction).update_rate_limit()):
            await interaction.send(f"**You will have to wait {(rta := int(retry_after))} second{'s'[:rta ^ 1]} to enable/disable scrobbling/logging.**", ephemeral=True)
            return
    ...
    async def send_authurl_callback(self, interaction: disnake.MessageInteraction):
        ...
        await interaction.send(f"### [Click here](<{self.auth_url}>) to link your Last.fm account (on the page click \"allow\")\n\n"
                               f"`The link expires in` <t:{self.last_timestamp}:R> `(If expired, click the button again).`\n\n"
                               f"`Warning: Do not show the \"click here\" link to anyone or send it in public "
                               f"places, as this link can grant access to your Last.fm account`\n\n"
                               "`If you have already authorized the application you must wait up to 20 seconds for the "
                               "message above to update confirming the process.`",
...
class LastFmCog(commands.Cog):
    ...
    @commands.command(hidden=True, name="lastfm", aliases=["lastfmconnect", "lfm"],
                      description="Link your Last.fm account to scrobble songs.",
    ...
    @commands.slash_command(hidden=True, name="lastfm",
                      description=f"{desc_prefix}Link your Last.fm account to scrobble songs.",
    ...
    async def lastfm(self, inter: disnake.ApplicationCommandInteraction):
        ...
        try:
            if not inter.permissions.embed_links:
                raise GenericError(f"**You do not have permission to insert links/attachments in channel <#{inter.channel_id}>**")
        ...
        if lastfm_user:
            ...
            txt += f"\n`Listen to your songs in a voice channel using the command` {slashcmd} `to register them on your " \
                    f"Last.fm account`\n"
            ...
            try:
                ...
                if top_tracks:
                    ...
                    embed = disnake.Embed(
                        ...
                        color=embed_color).set_author(name=f"Top 3: Most listened songs (out of {int(lastfm_user['track_count']):,} total):",
                ...
                if top_artists:
                    ...
                    embed = disnake.Embed(
                        ...
                        color=embed_color).set_author(name=f"Top 3: Most listened artists (out of {int(lastfm_user['artist_count']):,} total):",
                ...
                if top_albuns:
                    ...
                    embed = disnake.Embed(
                        ...
                        color=embed_color).set_author(name=f"Top 3: Most listened albums (out of {int(lastfm_user['album_count']):,} total):",
            ...
            embeds = [disnake.Embed(
                description="**Link (or create) an account on [last.fm](<https://www.last.fm/home>) to register "
                            ...
                            "songs/artists/albums and have a general statistic of the songs you've listened to, plus "
        ...
        if view.session_key:
            ...
            embeds[0].description += f"\n### The account [{view.username}](<https://www.last.fm/user/{view.username}>) has been " \
                                 "linked successfully!\n\n`Now when you listen to your songs in the voice channel they " \
                                " will be automatically registered on your Last.fm account`"
        ...
        if not (album := track.album_name) and track.info["sourceName"] in ("youtube", "soundcloud"):
            ...
            if track.ytid and track.author.endswith(" - topic") and not track.author.endswith("Release - topic") and not track.title.startswith(track.author[:-8]):
                ...
                if (fmdata := self.bot.last_fm.cache.get(track_query)) is None:
                    ...
                    if not result:
                        print(f"⚠️ - Last.FM Scrobble - No results for song: {track_query}")
