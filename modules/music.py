...
class Music(commands.Cog):
    ...
    @pool_command(
        ...
        description="Activate the channel's automatic announcement/status system with the song name.",
    ...
    async def set_voice_status(
        ...
        if not author.guild_permissions.manage_guild and not (await bot.is_owner(author)):
            raise GenericError("**You don't have server management permission to enable/disable this system.**")
        ...
        if not template:
            ...
            player.set_command_log(text="enabled automatic status", emoji="📢")
    ...
    @commands.slash_command(name="search", extras={"check_player": False}, cooldown=play_cd, max_concurrency=play_mc,
                            description=f"{desc_prefix}Search for music and select one of the results to play.")
    @commands.contexts(guild=True)
    async def search(
            self,
            ...
            *,
            ...
            server: str = commands.Param(name="server", desc="Use a specific music server for search.",
                                         default=None),
    ...
    async def do_connect(
        ...
        try:
            ...
            raise GenericError(
                f"**The player for bot {bot.user.mention} was terminated before connecting to the voice channel "
        ...
        if isinstance(ctx, disnake.ApplicationCommandInteraction) and ctx.application_command.name == self.connect.name:
            ...
            if channel != me.voice and me.voice.channel:
                txt = [
                    f"moved me to channel <#{channel.id}>",
                ...
                txt = [
                    f"connected me to channel <#{channel.id}>",
                    f"**Connected to channel** <#{channel.id}>"
                ]
        ...
        if deafen_check and bot.config["GUILD_DEAFEN_WARN"]:
            ...
            if not await check_deafen(me):
                await text_channel.send(
                    embed=disnake.Embed(
                        title="Notice:",
                        description="To maintain your privacy and help save resources, I recommend muting my audio in the channel by right-clicking me and selecting: mute audio on the server.",
        ...
        if isinstance(channel, disnake.StageChannel):
            ...
            if stage_perms.mute_members:
                ...
                embed.description = f"**I need a staff member to invite me to speak on the stage: " \
                                    f"[{channel.name}]({channel.jump_url}).**"

                embed.set_footer(
                    text="💡 Tip: to allow me to speak on stage automatically you must grant me the permission to mute members (either server-wide or just in the selected stage channel).")
    ...
    @pool_command(name="addposition", description="Add music at a specific position in the queue.",
                  aliases=["adp", "addpos"], check_player=False, cooldown=play_cd, max_concurrency=play_mc,
                  usage="{prefix}{cmd} [position(#)] [name|link]\nEx: {prefix}{cmd} 2 sekai - burn me down")
    ...
    stage_flags.add_argument('-next', action='store_true', help='Add the song/playlist to the top of the queue (equivalent to: -pos 1)')
    stage_flags.add_argument('-reverse', '-r', action='store_true', help='Reverse the order of added songs (only effective when adding a playlist).')
    stage_flags.add_argument('-shuffle', '-sl', action='store_true', help='Shuffle the added songs (only effective when adding a playlist).')
    stage_flags.add_argument('-select', '-s', action='store_true', help='Choose the song from the search results.')
    stage_flags.add_argument('-mix', '-rec', '-recommended', action="store_true", help="Add/play recommended songs with the given artist - song name.")
    stage_flags.add_argument('-force', '-now', '-n', '-f', action='store_true', help='Play the added song immediately (only effective if there is a song currently playing.)')
    ...
    async def play_file(
            ...
            file: disnake.Attachment = commands.Param(
                name="file", description="audio file to play or add to queue"
            ),
            position: int = commands.Param(name="position", description="Place the song in a specific position",
                                           default=0),
            force_play: str = commands.Param(
    ...
    async def play(
            ...
            mix: str = commands.Param(
                name="recommended",
                description="Play recommended songs based on the given artist - song name",
                default=False,
                choices=[
                    disnake.OptionChoice(disnake.Localized("Yes", data={disnake.Locale.pt_BR: "Sim"}), "yes"),
                ]
        ...
        """if not inter.author.voice:
            ...
            if not (c for c in guild.channels if c.permissions_for(inter.author).connect):
                raise GenericError(f"**You are not connected to a voice channel, and there are no voice channels/stages "
                                   "available on the server that grant you permission to connect.**")
            ...
            msg = await func(
                embed=disnake.Embed(
                    description=f"**{inter.author.mention} join a voice channel to play your music.**\n"
        ...
        if guild.me.voice and bot.user.id not in author.voice.channel.voice_states:
            ...
            if str(inter.channel.id) == guild_data['player_controller']['channel']:
                ...
                if len(free_bots) > 1 and manual_bot_choice == "yes":
                    ...
                    newmsg = await func(
                        embed=disnake.Embed(
                            description=f"**Choose which bot you want to use in channel {author.voice.channel.mention}**",
                    ...
                    if v.status is None:
                        try:
                            ...
                            func = inter.edit_original_message
                        try:
                            await func(embed=disnake.Embed(description="### Time expired...", color=self.bot.get_color(guild.me)), view=None)
                        except:
                            traceback.print_exc()
                        return
            ...
            if self.bot.config["ENABLE_DISCORD_URLS_PLAYBACK"]:
                ...
                try:
                    ...
                    if attachment.size > 18000000:
                        raise GenericError("**The file you sent must be 18mb or smaller.**")
        ...
        if not query:
            ...
            if user_data["fav_links"]:
                ...
                txt += f"`Play music or playlists that you've liked or added to your favorites.`\n"
            ...
            if user_data["integration_links"]:
                ...
                txt += f"`Play public playlists from a YouTube channel (or user profile from any music platform) from your integrations list.`\n"

            else:
                txt += f"`You don't have any integrations added... " \
                        f"Use integrations to add YouTube channel links (or user profile links from any music platform) for easy access to all their public playlists.`\n"

            txt += f"-# To manage your integrations use the command {fav_slashcmd} and then select the \"integrations\" option.\n" \
                    f"### `[💾] Saved Queue [💾]`\n"

            if os.path.isfile(f"./local_database/saved_queues_v1/users/{inter.author.id}.pkl"):
                txt += f"`Use music queue that you saved via command` {savequeue_slashcmd}.\n"
                opts.append(disnake.SelectOption(label="Use saved queue", value=">> [💾 Saved Queue 💾] <<", emoji="💾"))

            else:
                txt += "`You don't have a saved music queue`\n" \
            ...
            if user_data["last_tracks"]:
                txt += "### `[📑] Recent Songs [📑]`\n" \
                    "`Play a song that you recently listened to/added.`\n"
            ...
            if guild_data["player_controller"]["fav_links"]:
                txt += "### `[📌] Server Favorites [📌]`\n" \
                        "`Use server favorites (added by server staff).`\n"
            ...
            if not select_interaction or view.selected is False:
                ...
                embed.set_footer(text="⚠️ " + ("Selection time expired!" if view.selected is not False else "Canceled by user."))
        ...
        if query.startswith(">> [💠 Integrations 💠] <<"):
            ...
            if not user_data["last_tracks"]:
                raise GenericError("**You don't have songs registered in your history...**\n"
                                   "They will appear as you add songs via search or link.")
            ...
            if not guild_data["player_controller"]["fav_links"]:
                raise GenericError("**The server has no pinned/favorite links.**")
            ...
            if len(fav_opts) == 1:
                ...
                if menu == "favs":
                    embed.description = '### `[⭐] ⠂Favorites ⠂[⭐]`\n' \
                                        '`Play music or playlists that you\'ve liked or added to your favorites list.`\n' \
                    ...
                    embed.description = '### `[💠] ⠂Integrations ⠂[💠]`\n' \
                                        '`Play public playlists from a YouTube channel (or a user profile from any music platform) from your integrations list.`\n' \
                                        f'-# To manage your integrations use the command {fav_slashcmd} and then select the \"integrations\" option.\n\n' \
                    ...
                    embed.description = f'### 📑 ⠂Play recent music/playlists:\n{embed.description}\n\n**Select an item below:**'
                ...
                if not select_interaction or view.selected is False:
                    ...
                    embed.set_footer(text="⚠️ Time expired!" if not view.selected is False else "⚠️ Canceled by user.")
        ...
        if (matches := spotify_regex_w_user.match(query)):
            ...
            if not self.bot.spotify:
                raise GenericError("**Spotify support is not available at the moment...**")
            ...
            if not (info := self.bot.pool.integration_cache.get(query)):
                ...
                try:
                    if not info["entries"]:
                        pprint.pprint(info)
                        raise GenericError(f"**Content unavailable (or private):**\n{query}")
                except KeyError:
            ...
            if urls:
                ...
                if query.startswith("https://www.youtube.com/results"):
                    try:
                        ...
                        raise GenericError(f"**There is no support for the provided link:** {query}")
                    manual_selection = True
                    ...
                    view = ButtonInteraction(
                        ...
                        buttons=[
                            disnake.ui.Button(label="Load music only", emoji="🎵", custom_id="music"),
                            disnake.ui.Button(label="Load playlist", emoji="🎶", custom_id="playlist"),
                        ]
                    )

                    embed = disnake.Embed(
                        description='**The link contains a video with playlist.**\n'
                                    f'Select an option within <t:{int((disnake.utils.utcnow() + datetime.timedelta(seconds=45)).timestamp())}:R> to continue.\n'
                                    f'-# Note: If the link is from a private playlist, only the current video will be loaded.',
                        color=self.bot.get_color(guild.me)
                    )
                ...
                embed = disnake.Embed(
                    ...
                    description="**Which service do you want to prioritize for the music search?**\n"
                                "-# Note: If the desired song is not found in the chosen service, another service will be used automatically.\n"
                                f'-# Note 2: If you don\'t select an option below <t:{int((disnake.utils.utcnow() + datetime.timedelta(seconds=45)).timestamp())}:R>, the default service will be used automatically.'
                )
    ...
    case_sensitive_args.add_argument('-casesensitive', '-cs', action='store_true',
                             help="Search for songs using the exact phrase in the song name instead of word-by-word.")
    @check_stage_topic()
    @check_yt_cooldown()
    @is_requester()
    @check_queue_loading()
    @check_voice()
    @pool_command(name="skip", aliases=["next", "n", "s", "skipto"], cooldown=skip_back_cd,
    ...
    async def skipto(
            ...
            case_sensitive: bool = commands.Param(
                ...
                description="Search for songs using the exact phrase in the song name instead of word-by-word.",
    ...
    async def skip(
            ...
            play_only: str = commands.Param(
                ...
                description="Only play the music immediately (without rotating the queue)",
                default="no"
            ),
            case_sensitive: bool = commands.Param(
                name="nome_exato", default=False,
        ...
        if query:
            ...
            embed = disnake.Embed(
                color=self.bot.get_color(guild.me),
                description= f"⤵️ **⠂{inter.author.mention} skipped to song:**\n"
    ...
    @has_player()
    @check_voice()
    @pool_command(name="back", aliases=["b"], description="Go back to the previous song.", only_voiced=True,
    ...
    async def back(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if isinstance(inter, disnake.MessageInteraction):
            interaction = inter
            player.set_command_log(text=f"{inter.author.mention} went back to the current song.", emoji="⏮️")
            await inter.response.defer()
        else:
            ...
            txt = [
                "went back to the current song.",
                f"⏮️ **⠂{inter.author.mention} went back to song:\n╰[`{fix_characters(t.title, 43)}`](<{t.uri or t.search_uri}>)**"
            ]
    ...
    async def volume(
        ...
        if value is None:
            ...
            raise GenericError("The volume must be between **5** and **150**.")
    ...
    @has_source()
    @check_voice()
    @pool_command(name="pause", aliases=["pausar"], description="Pause the song.", only_voiced=True,
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Pause the song.", extras={"only_voiced": True},
        cooldown=pause_resume_cd, max_concurrency=pause_resume_mc
    )
    @commands.contexts(guild=True)
    async def pause(self, inter: disnake.ApplicationCommandInteraction):
        ...
        txt = ["paused the song.", f"⏸️ **⠂{inter.author.mention} paused the song.**"]
    ...
    @has_source()
    @check_voice()
    @pool_command(name="resume", aliases=["unpause"], description="Resume/Unpause the song.", only_voiced=True,
                  cooldown=pause_resume_cd, max_concurrency=pause_resume_mc)
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Resume/Unpause the song.",
    ...
    async def resume(self, inter: disnake.ApplicationCommandInteraction):
        ...
        txt = ["resumed the song.", f"▶️ **⠂{inter.author.mention} unpaused the song.**"]
        await self.interaction_message(inter, txt, rpc_update=True, emoji="▶️")
        await player.update_stage_topic()
    ...
    @has_source()
    @check_voice()
    @pool_command(name="seek", aliases=["sk"], description="Forward/Resume the song to a specific time.",
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Seek/Resume the song to a specific time.",
    ...
    async def seek(
            self,
            inter: disnake.ApplicationCommandInteraction,
            position: str = commands.Param(name="time", description="Time to forward/rewind (ex: 1:45 / 40 / 0:30)")
    ):
        ...
        if seconds is None:
            raise GenericError(
                "**You used an invalid time! Use seconds (1 or 2 digits) or format (minutes):(seconds)**")
        ...
        if milliseconds > player.position:
            ...
            txt = [
                f"forwarded the song time to: `{time_format(milliseconds)}`",
                f"{emoji} **⠂{inter.author.mention} forwarded the song time to:** `{time_format(milliseconds)}`"
            ]
            ...
            txt = [
                f"rewound the song time to: `{time_format(milliseconds)}`",
                f"{emoji} **⠂{inter.author.mention} rewound the song time to:** `{time_format(milliseconds)}`"
            ]
    ...
    @check_voice()
    @pool_command(
        description=f"Select repeat mode between: current song / queue / disable / amount (using numbers).",
    ...
    async def loop(self, ctx: CustomContext, mode: str = None):
        ...
        if mode.isdigit():
            ...
            if len(mode) > 2 or int(mode) > 10:
                raise GenericError(f"**Invalid amount: {mode}**\n"
                                   "`Maximum amount allowed: 10`")
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Select repeat mode between: current / queue or disable.",
    ...
    async def loop_mode(
        ...
        if mode == 'off':
            ...
            emoji = "⭕"
            txt = ['disabled repeat.', f"{emoji} **⠂{inter.author.mention}disabled repeat.**"]

        elif mode == "current":
            player.current.info["extra"]["track_loops"] = 0
            emoji = "🔂"
            txt = ["enabled current song repeat.",
                   f"{emoji} **⠂{inter.author.mention} enabled current song repeat.**"]
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Set the number of repeats for the current song.",
        extras={"only_voiced": True}, cooldown=loop_cd, max_concurrency=loop_mc
    )
    @commands.contexts(guild=True)
    async def loop_amount(
            self,
            inter: disnake.ApplicationCommandInteraction,
            value: int = commands.Param(name="value", description="number of repeats.")
    ):
    ...
    async def remove_legacy(self, ctx: CustomContext, *, flags: str = ""):
        ...
        if not unknown:
            raise GenericError("**You didn't provide a song name.**")
    ...
    @has_player()
    @check_voice()
    @pool_command(name="readd", aliases=["rdd"], only_voiced=True, cooldown=queue_manipulation_cd,
                  max_concurrency=remove_mc, description="Re-add played songs to the queue.")
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Re-add played songs to the queue.",
    ...
    async def readd_songs(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not player.played and not player.failed_tracks:
            raise GenericError("**No played songs.**")
    ...
    async def song_request_thread(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not self.bot.intents.message_content:
            raise GenericError("**Currently, I don't have the message-content intent to check "
                               "message contents**")
        ...
        if player.has_thread:
            raise GenericError("**There is already an active thread/conversation in the player.**")
        ...
        if not player.controller_mode:
            raise GenericError("**The current skin/appearance is not compatible with the song-request system "
                               "via thread/conversa\n\n"
        ...
        if not [m for m in player.guild.me.voice.channel.members if not m.bot and
            ...
            raise GenericError(f"**There are no members in channel <#{player.channel_id}> with permission to send messages "
                               f"in topics in channel {player.text_channel.mention}")
        ...
        txt = [
            "Enabled temporary thread/conversation system for song requests.",
    ...
    @pool_command(name="nightcore", aliases=["nc"], only_voiced=True, cooldown=nightcore_cd, max_concurrency=nightcore_mc,
                  description="Enable/Disable nightcore effect (Accelerated music with higher pitch).")
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Enable/Disable nightcore effect (Accelerated music with higher pitch).",
    ...
    async def now_playing(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not player.current:
            raise GenericError(f"**I'm not currently playing anything in channel {player.last_channel.mention}**")
        ...
        if player.guild_id != inter.guild_id:
            ...
            txt += f"> ⏱️ **⠂Player active:** <t:{player.uptime}:R>\n"
        ...
        if player.queue or player.queue_autoplay:
            ...
            if player.guild_id == inter.guild_id:
                ...
                if qsize > 3:
                    components.append(
                        disnake.ui.Button(custom_id=PlayerControls.queue, label="View complete list",
        ...
        if player.static and player.guild_id == inter.guild_id:
            if player.message:
                ...
                txt += f"\n\n`Access the player-controller in the channel:` {player.text_channel.mention}"

        embed = disnake.Embed(description=txt, color=self.bot.get_color(player.guild.me))

        embed.set_author(name=("⠂Playing Now:" if inter.guild_id == player.guild_id else "You listening now:") if not player.paused else "⠂Current Track:",
        ...
        if channel != player.text_channel:
            ...
            try:
                ...
                player.set_command_log(
                    text=f"{inter.author.mention} moved the player controller to channel {inter.channel.mention}.",
                    emoji="💠"
                )

                embed = disnake.Embed(
                    description=f"💠 **⠂{inter.author.mention} moved the player controller to channel:** {channel.mention}",
                    color=self.bot.get_color(guild.me)
                )
    ...
    async def remove_dj(
        ...
        if user.id == player.player_creator:
            ...
            GenericError(f"Member {user.mention} is not in the DJ list")
    ...
    @has_player()
    @check_voice()
    @pool_command(name="stop", aliases=["leave"], only_voiced=True,
    ...
    @pool_command(
        ...
        description="Experimental: Save the current track and queue to reuse them at any time."
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Experimental: Save the current track and queue to reuse them at any time.",
        extras={"only_voiced": True}, cooldown=queue_manipulation_cd, max_concurrency=remove_mc
    )
    @commands.contexts(guild=True)
    async def save_queue(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if len(tracks) < 3:
            raise GenericError(f"**You need at least 3 tracks to save (current and/or in queue)**")
        ...
        embed = disnake.Embed(
            ...
            description=f"### {inter.author.mention}: The queue was saved successfully!!\n"
                        f"**Saved songs:** `{len(tracks)}`\n"
                        "### How to use?\n"
                        f"* Use the {slashcmd} command (selecting from search autocomplete)\n"
                        "* Click the player's favorite/integration play button/select.\n"
                        f"* Use the command {global_data['prefix'] or self.bot.default_prefix}{self.play_legacy.name} "
                        "without including a song/video name or link."
        )

        embed.set_footer(text="Note: This is a very experimental feature; the saved queue may change or be removed in future updates")
    ...
    adv_queue_flags.add_argument('-uploader', '-author', '-artist', nargs='+', default=[],
                                 help="Remove songs with the specified author/artist/uploader name.\nEx: -uploader sekai")
    ...
    adv_queue_flags.add_argument('-playlist', '-list', '-pl', nargs='+', default=[],
                                 help="Remove songs with the specified name in the associated playlist.\nEx: -playlist myplaylist")
    ...
    adv_queue_flags.add_argument('-startposition', '-startpos', '-start', type=int, default=0,
                                 help="Remove songs starting from a specific position in the queue.\nEx: -start 10")
    adv_queue_flags.add_argument('-endposition', '-endpos', '-end', type=int, default=0,
                                 help="Remove songs from the queue up to a specific position.\nEx: -end 15")
    adv_queue_flags.add_argument('-absentmembers', '-absent', '-abs', action='store_true',
                                 help="Remove songs added by members who left the channel")
    ...
    async def clear(
        ...
        if not filters and not range_start and not range_end:
            ...
            try:
                ...
                txt.append("`Tracks requested by members who left the channel.`")
            except:
                pass
    ...
    async def do_move(
        ...
        if not tracklist:
            raise GenericError("No music found with the selected filters!")
                ...
                if channel_db:
                    ...
                    if not send_message_perm:
                        raise GenericError(
                            ...
                            "If you want to reset the music request channel configuration, use the /reset or /setup command "
                            "again..."
                        )

                    if not channel_db_perms.embed_links:
                        raise GenericError(
                            f"**{bot.user.mention} does not have permission to attach links/embeds in the channel <#{static_player['channel']}>**\n"
                            "If you want to reset the music request channel configuration, use the /reset or /setup command "
                            "again..."
                        )
    ...
    async def player_controller(self, interaction: disnake.MessageInteraction, control: str, **kwargs):
            ...
            try:
                ...
                if control == PlayerControls.embed_enqueue_playlist:
                    ...
                    if (retry_after := self.bot.pool.enqueue_playlist_embed_cooldown.get_bucket(interaction).update_rate_limit()):
                        raise GenericError(
                            f"**You need to wait {(rta:=int(retry_after))} second{'s'[:rta^1]} to add a playlist to the current player.**")
                    ...
                    await interaction.send(f"{interaction.author.mention}, the playlist [`{result.name}`](<{url}>) has been successfully added!{player.controller_link}", ephemeral=True)
        ...
        if control == PlayerControls.embed_add_fav:
            ...
            if self.bot.config["MAX_USER_FAVS"] > 0 and not (await self.bot.is_owner(interaction.author)):
                ...
                if (current_favs_size + (user_favs := len(user_data["fav_links"]))) > self.bot.config["MAX_USER_FAVS"]:
                    await interaction.edit_original_message(
                        "You don't have enough space to add all favorites from your file...\n"
            ...
            await interaction.send(embed=disnake.Embed(
                description=f"[`{fav_name}`](<{embed.author.url}>) **was added to your favorites!**\n\n"
                            "**How to use?**\n"
                            f"* Using the {cmd} command (selecting the favorite in search autocomplete)\n"
                            "* Click the player's favorite/integration play button/select.\n"
                            f"* Use the command {global_data['prefix'] or self.bot.default_prefix}{self.play_legacy.name} without including a song/video name or link.\n"
        ...
        try:
            ...
            if control == PlayerControls.add_favorite:
                ...
                if not player.current:
                    await interaction.send("**There is no track currently playing...**", ephemeral=True)
                    return
                ...
                if not choices:
                    ...
                    await interaction.send(
                        embed=disnake.Embed(
                            color=self.bot.get_color(interaction.guild.me),
                            description="### No items to favorite in the current track."
                        ), ephemeral=True
                    )
                    return
            ...
            if control == PlayerControls.lyrics:
                ...
                not_found_msg = "No lyrics available for the current track..."
    ...
    async def process_music(
        ...
        if not player.current:
            ...
            player.set_command_log(
                emoji="▶️",
                text=f"{inter.author.mention} added the current track to play immediately."
            )
            await player.track_end()
            await player.process_next()
    ...
    async def node_reconnect(self, node: wavelink.Node):
        ...
        if node._retry_count >= 4:
            print(f"❌ - {self.bot.user} - [{node.identifier} / v{node.version}] Reconnection cancelled.")
            node._retry_count = 0
            return
        else:
            ...
            if not tracks:
                ...
                if txt:
                    if "This track is not readable. Available countries:" in txt:
                        txt = "The specified track is not available in my current region..."
    ...
    async def thread_song_request(self, thread: disnake.Thread, reopen: bool = False, bot: BotCore = None):
        ...
        if not thread.parent.permissions_for(thread.guild.me).send_messages_in_threads:
            await player.text_channel.send(
                embed=disnake.Embed(
                    ...
                    description="**I don't have permission to send messages in the current channel's conversations to activate "
        ...
        if not bot.intents.message_content:
            ...
            if reopen:
                ...
                embed.description = "**This conversation will be temporarily used for song requests.**\n\n" \
    ...
    async def check_available_bot(self, inter, guild: disnake.Guild, bot: BotCore = None, message: disnake.Message = None):
        ...
        if not free_bots:
            ...
            if bot_count:
                ...
                txt = "**There are no compatible music bots in the server...**" \
