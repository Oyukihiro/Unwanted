...
class Misc(commands.Cog):
    ...
    async def guild_add(self, guild: disnake.Guild):
        ...
        if cmd:=self.bot.get_command("setup"):
            cmd_text = f"If you wish, use the **/{cmd.name}** command to create a dedicated channel to request " \
                        "songs without commands and keep the music player fixed in a dedicated channel.\n\n"
        else:
            cmd_text = ""
                ...
                async for entry in guild.audit_logs(action=disnake.AuditLogAction.integration_create, limit=50):
                    ...
                    if entry.target.application_id == self.bot.user.id:
                        ...
                        embeds.append(
                            disnake.Embed(
                                color=color,
                                description=f"To see all my commands use slash (**/**) on the server " \
                                             f"**{guild.name}**"
                            ).set_image(url=image)
                        )
                        ...
                        embeds.append(
                            disnake.Embed(
                                ...
                                description=f"I also have text commands by prefix. {prefix_msg} (my mention "
                                            ...
                                            f"use **{prefix}help** in a channel on the server **{guild.name}**. "
                        ...
                        if bots_in_guild:
                            ...
                            msg = f"I noticed there are other bots on the server **{guild.name}** that I'm compatible with " \
                                   ...
                                   f"When using music commands (e.g., play) without one of the bots connected to the channel, " \
                                    "one of the free bots on the server will be used."
                            ...
                            send_video = "**If there is demand on your server you can also add more extra music bots.\n" \
                                         ...
                                         f"to memorize prefixes and slash commands for each bot individually.\n\n" \
        ...
        embeds = [
            disnake.Embed(
                color=color, description="Hello! To see all my commands use slash (**/**)\n"
                                         ...
                                         "perhaps it has reached the limit of bots with slash commands "
        ...
        if bots_in_guild:
            ...
            msg = f"I noticed there are other bots on the server **{guild.name}** that I'm compatible with " \
                  ...
                  f"When using music commands (e.g., play) without one of the bots connected to the channel, one " \
                   f"of the free bots on the server will be used."
        ...
        try:
            ...
            if send_video:
                ...
                await asyncio.sleep(1)
                await channel.send(f"{send_video}\n\nCheck out the [**video**]({self.bot.config['MULTIVOICE_VIDEO_DEMO_URL']}) demonstrating this functionality.", **kwargs)
        except:
    ...
    async def about(
        ...
        links = "[`[YouTube]`](https://www.youtube.com/@unwanted_arc)"
    ...
    async def invite_button(self, inter: disnake.MessageInteraction, is_command=False):
        ...
        if bots_in_guild:
            txt += "## Music bots already on the current server:\n"
    ...
    @commands.command(name="invite", aliases=["inv"], description="Display my invite link so you can add me to your server.")
    async def invite_legacy(self, ctx):
        await self.invite.callback(self=self, inter=ctx)


    @commands.slash_command(
        description=f"{desc_prefix}Display my invite link so you can add me to your server.",
        extras={"allow_private": True}
    )
    @commands.contexts(guild=True)
    ...
    async def fixfavs(self, ctx: CustomContext):
        ...
        async with ctx.typing():
            ...
            await ctx.send("Favorites have been successfully fixed!")
...
class GuildLog(commands.Cog):
    ...
    def __init__(self, bot: BotCore):
        ...
        if bot.config["BOT_ADD_REMOVE_LOG"]:
            ...
            if URL_REG.match(bot.config["BOT_ADD_REMOVE_LOG"]):
                ...
                print("Invalid webhook URL (for sending logs when adding/removing bot).")
    ...
    async def on_guild_remove(self, guild: disnake.Guild):
        ...
        try:
            await self.send_hook(guild, title="I was removed from the server", color=disnake.Color.red())
        except:
            traceback.print_exc()
    ...
    async def on_guild_join(self, guild: disnake.Guild):
        ...
        try:
            await self.send_hook(guild, title="I was added to a new server", color=disnake.Color.green())
        except:
            traceback.print_exc()
