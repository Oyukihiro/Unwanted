...
# note: this system is entirely experimental.
import asyncio
import os
import pickle
import shutil
...
class PlayerSession(commands.Cog):
        ...
        try:
            ...
            if self.bot.config["PLAYER_SESSIONS_MONGODB"] and self.bot.config["MONGO"]:
                for d in local_sessions:
                    data_list[d["_id"]] = d
                    print(f"{self.bot.user} - Migrating session data from server: {d['_id']} | Local DB -> Mongo")
                ...
                for d in mongo_sessions:
                    data_list[d["_id"]] = d
                    print(f"{self.bot.user} - Migrating session data from server: {d['_id']} | Mongo -> Local DB")
    ...
    async def voice_check(self, voice_channel: Union[disnake.VoiceChannel, disnake.StageChannel], position: int = 0):
        ...
        if isinstance(voice_channel, disnake.StageChannel) and \
            ...
            try:
                ...
                print(f"{self.bot.user} - Failed to speak in stage in server {guild.name}. Error: {repr(e)}")
    ...
    async def resume_player(self, data: dict, hints: list = None):
        ...
        try:
            ...
            if not guild:
                print(f"{self.bot.user} - Player Ignored: {data['_id']} | Non-existent server...")
                return
            ...
            try:
                ...
                if text_channel:
                    ...
                    if text_channel:
                        try:
                            ...
                            print(f"{self.bot.user} - Controller Ignored (permission denied) [Channel: {text_channel.name} | ID: {text_channel.id}] - [ {guild.name} - {guild.id} ]")
                            text_channel = None
                        else:
                            if data["message_id"]:
                                try:
                ...
                if not voice_channel:
                    ...
                    try:
                        msg = "Player ended because the voice channel does not exist or was deleted."
                ...
                try:
                    ...
                    print(f"{self.bot.user} - Player Ignored: {guild.name} [{guild.id}]\n{repr(e)}")
                    try:
                        msg = f"The player was ended due to lack of permission to connect to channel {voice_channel.mention}."
                        if not data["skin_static"]:
                            await text_channel.send(embed=disnake.Embed(description=msg, color=self.bot.get_color(guild.me)))
                        else:
                            await send_idle_embed(text_channel, bot=self.bot, text=msg)
            ...
            if started:
                player.set_command_log(
                    text="Player data restored successfully!",
                    emoji="🔰"
                )
                player.update = True
                ...
                player.set_command_log(
                    text="Player restored successfully!",
                    emoji="🔰"
                )
            ...
            print(f"{self.bot.user} - Critical failure to resume players:\n{traceback.format_exc()}")
    ...
    async def save_session(self, player: LavalinkPlayer, data: dict):
        ...
        try:
            ...
            print(f"❌ - {self.bot.user} - Save canceled: {repr(e)}")
