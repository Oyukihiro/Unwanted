...
class IndexHandler(tornado.web.RequestHandler):
    ...
    async def get(self):
        ...
        if pending_bots:
            msg += f"\n<p style=\"font-size:20px\">Bots Starting Up:</p>" \
        ...
        msg += f"<p><a href=\"https://github.com/zRitsu/DC-MusicBot-RPC" \
              f"/releases\" target=\"_blank\">Download the rich presence app here.</a></p>Link to add in the app " \
              f"RPC: {ws_url}"
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if not ws_id:
            ...
            if not bot_id:
                print(f"disconnecting: due to missing user id {self.request.remote_ip}\nData: {data}")
                self.write_message(json.dumps({"op": "disconnect", "reason": "Disconnecting due to missing user ids"}))
                ...
                if self.auth_enabled:
                    ...
                    if users_ws[data["user"]].token != token:
                        ...
                        data.update(
                            {
                                "op": "exception",
                                "message": "invalid token! Just to be safe, generate a new token using the command in "
                                           "bot: /rich_presence."
                            }
                        )
        ...
        if app_version < minimal_version:
            self.write_message(json.dumps({"op": "disconnect", "reason": "Unsupported app version! Make sure you are using "
        ...
        if len(token) not in (0, 50):
            self.write_message(
                json.dumps({"op": "disconnect", "reason": f"The token must have 50 characters..."}))
            self.close(code=4200)
            return
    ...
    def on_close(self):
        ...
        if self.user_ids:
            print("\n".join(f"👤 - Connection Ended - User: {u}" for u in self.user_ids))
        ...
        if not self.bot_ids:
            print(f"Connection Ended - IP: {self.request.remote_ip}")

        else:

            print(f"🌐 - Connection Ended - Bot ID's: {self.bot_ids}")
    ...
    async def ws_loop(self):
        ...
        while True:
            ...
            if message.type in (aiohttp.WSMsgType.CLOSED, aiohttp.WSMsgType.ERROR):
                ...
                print(f"🌐 - RPC Websocket Ended: {message.extra}")
                return
